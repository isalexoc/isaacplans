/**
 * Shared, pure format validators for ACA intake. No React / DOM — safe to import on the
 ...
 *
 * Unlike the IUL copy this mirrors, zip is driven by the `zip` field type rather than
 * inferred from `digitsOnly && maxLength === 5`, and there is no hardcoded age-key set.
 */

import type { AcaField } from "./fields";
 ...
 */
export function fieldFormatError(field: AcaField, value: string): FieldErrorKey | null {
  const v = (value ?? "").trim();
  if (!v) return null;
