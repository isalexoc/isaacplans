/* components/form-modal-health-alternative.tsx — Health Coverage Alternative CTA: simple lead capture + Agent CRM */
"use client";
...
import HealthAlternativeLeadForm from "@/components/health-alternative-lead-form";
...
export const HealthAlternativeQuoteModal = ({ open, setOpen }: Props) => {
  const isES = useLocale().startsWith("es");

  useEffect(() => {
    if (open) {
      trackInitiateCheckout({
        contentName: "Health Coverage Alternative Quote Request",
      });
    }
  }, [open]);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="flex flex-col min-h-0 flex-1 w-full rounded-xl bg-background shadow-2xl overflow-hidden">
          <div className="flex-shrink-0 z-10 flex items-center justify-between gap-3 px-4 py-3 bg-gradient-to-r from-blue-600/12 to-indigo-600/10 dark:from-blue-900/25 dark:to-indigo-900/20 border-b border-blue-200/40 dark:border-blue-800/50">
            ...
            <div className="font-semibold flex-1 text-center text-gray-900 dark:text-white text-sm sm:text-base">
              {isES ? "Cotiza tu Cobertura de Salud" : "Health Coverage Quote"}
            </div>
            <button
              type="button"
          ...
          <div className="flex-1 min-h-0 overflow-y-scroll overflow-x-hidden overscroll-y-contain scrollbar-none">
            <div className="flex flex-col sm:flex-row min-h-0">
              <div className="hidden md:flex flex-col w-full sm:w-[220px] lg:w-[260px] flex-shrink-0 items-center justify-start gap-0 pt-8 pb-6 px-6 bg-gradient-to-b from-blue-500/06 to-transparent dark:from-blue-900/15 dark:to-transparent border-r border-gray-100 dark:border-gray-800">
              ...
              <div className="flex-1 min-w-0 px-4 sm:px-6 py-6">
                <div className="max-w-[400px] mx-auto">
                  <HealthAlternativeLeadForm onCloseModal={() => setOpen(false)} />
                </div>
              </div>
            </div>
          </div>
