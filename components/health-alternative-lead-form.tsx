...
import {
  submitHealthAlternativeForm,
  type ActionResult,
  type FormValues,
} from "@/app/actions/health-alternative";
import PhoneInput, { parsePhoneNumber } from "react-phone-number-input";
import "react-phone-number-input/style.css";
import { Link } from "@/i18n/navigation";
import HealthAlternativeSelfEnrollCta from "@/components/health-alternative-self-enroll-cta";
...
export default function HealthAlternativeLeadForm({ onSubmitSuccess, onCloseModal }: Props) {
  const locale = useLocale();
  const isES = locale.startsWith("es");
  const t = useTranslations("healthAlternativePage.leadForm");
  ...
  if (state.status === "SUCCESS") {
    return (
      <div className="py-4 text-center">
        <div className="rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/30 dark:to-indigo-900/20 border-2 border-blue-200 dark:border-blue-800 p-6 shadow-sm">
          <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-blue-500/20 dark:bg-blue-500/30 flex items-center justify-center">
            <svg
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              href={
                ...
                formValues.email ||
                formValues.phone
                  ? `/health-alternative/calendar?${new URLSearchParams({
                      first_name: formValues.firstName,
                      last_name: formValues.lastName,
                      email: formValues.email,
                      phone: formValues.phone,
                    }).toString()}`
                  : "/health-alternative/calendar") as "/health-alternative/calendar"
              }
          ...
          <HealthAlternativeSelfEnrollCta />
        </div>
      </div>
    );
  }
  ...
  return (
    <form id="health-alternative-lead-form" action={formAction} className="space-y-4">
      ...
      <div>
        <label
          htmlFor="health-alternative-cta-firstName"
        ...
        <input
          type="text"
          id="health-alternative-cta-firstName"
          name="firstName"
      ...
      <div>
        <label
          htmlFor="health-alternative-cta-lastName"
          className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5"
        >
        ...
        <input
          type="text"
          id="health-alternative-cta-lastName"
          name="lastName"
      ...
      <div>
        <label
          htmlFor="health-alternative-cta-email"
          className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5"
        >
        ...
        <input
          type="email"
          id="health-alternative-cta-email"
          name="email"
      ...
      <div>
        <label
          htmlFor="health-alternative-cta-phone"
          className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5"
        >
          {t("phone")} <span className="text-red-500">*</span>
        </label>
        <PhoneInput
          id="health-alternative-cta-phone"
          name="phone"
          defaultCountry="US"
          countries={["US"]}
      ...
      <button
        type="submit"
        disabled={isPending}
        className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold py-3.5 px-6 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
      >
