...
import { Contact, ExternalLink, Loader2, Phone, Shield, ShieldCheck } from "lucide-react";
...
const LIFE_INSURANCE_LEAD_VALUE = 100;
...
export default function LifeInsuranceGetCoveredFunnel({
  ...
  const t = useTranslations("lifeInsuranceGetCoveredPage.funnel");
  ...
  const calendarBookingHref = (() => {
    ...
    return appendAgentCrmBookingPrefill("/life-insurance/calendar", {
    ...
    try {
      ...
      const res = await fetch("/api/create-contact", {
        ...
        body: JSON.stringify({
          ...
          lifeInsuranceData: {
            language: isES ? "es" : "en",
            source: "life_insurance_get_covered_ads",
            smsConsent,
            marketingConsent,
          },
          meta: {
      ...
      if (process.env.NODE_ENV === "development" && capiDispatched !== true) {
        console.warn(
          "[life-insurance/get-covered] Meta CAPI Lead was not dispatched. Set META_CAPI_ACCESS_TOKEN and NEXT_PUBLIC_FACEBOOK_PIXEL_ID; ensure meta.eventId and eventSourceUrl are sent (also sent on duplicate-merge for the life_insurance_get_covered_ads source)."
        );
      }
      ...
      trackLead(
        {
          contentName: "Life Insurance — get covered",
          value: LIFE_INSURANCE_LEAD_VALUE,
          currency: "USD",
          source: "life_insurance_get_covered_ads",
        },
        eventId
                  ...
                  <div className="mt-4 flex w-full flex-col gap-3">
                    <div>
                      <Button
                        ...
                        className="h-16 w-full rounded-xl bg-gradient-to-r from-[hsl(var(--custom))] to-blue-600 text-lg font-semibold text-white shadow-lg"
                      >
                        <Link href={calendarBookingHref as "/life-insurance/calendar"}>
                          {t("done.bookCta")}
                        </Link>
                      </Button>
                    </div>
