 ...
 * localized start path. Mirrors components/aca-apply-cta.tsx.
 */
...
export default function FinalExpenseApplyCta({ label, startHref }: { label: string; startHref: string }) {
  const { isLoaded, isSignedIn } = useUser();

  if (isLoaded && isSignedIn) {
    return (
      <Button asChild size="lg" className={CTA_CLS}>
        <Link href="/final-expense/apply/start">
