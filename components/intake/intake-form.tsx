...
import {
  ...
  removeIntakeFile,
  type FileTargetRef,
} from "@/lib/intake-core/api-client";
...
import { fieldFormatError, type FieldErrorKey } from "@/lib/intake-core/validation";
import { digitsToStored, formatMoneyDisplay } from "@/lib/iul-intake/money";
import { sectionMissingFields } from "@/lib/intake-core/schema";
...
/**
 * Icon per section, shown in a colored chip next to the step title. Keyed by the section `key`
 * a config declares; anything unlisted falls back to a generic one at the render site.
 */
...
/**
 * Add-button copy for a repeater — "Add another person" from the field's own row label, so a new
 * line of business gets sensible copy without touching this file.
 */
function addLabelFor(field: IntakeField, locale: IntakeLocale): string {
  const noun = locale === "es" ? field.rowLabelEs : field.rowLabelEn;
  if (!noun) return tr(UI.addRow, locale);
  return locale === "es" ? `Agregar ${noun.toLowerCase()}` : `Add ${noun.toLowerCase()}`;
}

/* ------------------------------- component ------------------------------- */

export default function IntakeForm({ lob, token }: { lob: string; token: string }) {
  ...
  const { status: saveStatus } = useIntakeAutosave({ lob, token, data });

  const isOwner = session?.role === "owner";
  const sections = useMemo(
    () => visibleSections(config.sections, !!isOwner),
    [config.sections, isOwner]
  );
  ...
  function setRowField(repeaterKey: string, index: number, subKey: string, value: unknown) {
    setData((prev) => {
      const field = fieldByKey(config.sections, repeaterKey);
  ...
  function validateOnBlur(field: IntakeField, value: string, key: string) {
    ...
    const labels = keys
      .map((k) => {
        ...
        if (rowMatch) {
          ...
          const who = `${rowLabel(repeater ?? ({} as IntakeField), locale)} ${Number(indexRaw) + 1}`.trim();
          return sub ? `${who}: ${fieldLabel(sub, locale)}` : who;
        }
        if (k === "householdMembers") return tr(UI.needOneMember, locale);
        const f = fieldByKey(config.sections, k);
  ...
  async function handleFinish() {
    ...
    setCompleteError(null);
    try {
      const result = await completeIntake(lob, token);
      if (result.success) {
        setCompleted(true);
        setMissing(new Set());
      } else {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      {isOwner && <IntakeBreadcrumb lob={lob} current={tr(UI.navForm, locale)} />}
      {/* Out-of-flow save status — fixed so it never shifts the page layout. */}
      <SaveIndicator status={saveStatus} locale={locale} />
      <h1 className="mb-1 text-2xl font-extrabold tracking-tight sm:text-3xl">{config.formTitle[locale]}</h1>
                ...
                if (field.type === "repeater") {
                  return (
                    <RepeaterEditor
                      ...
                      addLabel={addLabelFor(field, locale)}
                    />
                  );
                }
...
/**
 * Config-driven repeater. Renders each row's `rowFields` through the same `FieldInput` dispatch as
 * top-level fields, with two capabilities a flat list of inputs could not give us:
 *
 *  1. `showIf` resolves against the ROW, so a per-person branch (has an SSN → number, else a
 *     reason) works independently on every row.
 *  2. `file` sub-fields upload against `repeaterKey` + `rowIndex`, so each row carries its own
 *     documents into the shared CRM bucket.
 */
function RepeaterEditor({
  ...
  onBlurSub: (field: IntakeField, value: string, key: string) => void;
  isOwner: boolean;
  reveal: boolean;
  addLabel: string;
}) {
  ...
  return (
    <div className={`space-y-4 ${invalidBlock ? "rounded-md border border-red-500 p-3" : ""}`}>
      {list.map((row, index) => {
        ...
        const heading = `${rowLabel(field, locale)} ${index + 1}`;
          ...
          <div key={index} className="rounded-md border p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {(field.rowFields ?? []).map((sub) => {
                // Row-scoped conditional visibility — per-person branches live here.
...
function FileUploader({
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    try {
      ...
      for (const original of Array.from(fileList)) {
        ...
        setBusy("uploading");
        const res = await uploadIntakeFile(lob, token, target, prepared);
        latest = res.files;
      }
      onChange(latest);
    } catch (e) {
  ...
  async function handleRemove(url: string) {
    try {
      const res = await removeIntakeFile(lob, token, target, url);
      onChange(res.files);
    } catch {
