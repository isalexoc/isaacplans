...
import type { IntakeSummary, IntakeStatus } from "@/lib/intake-core/types";
...
export default function IntakeDashboard({ lob }: { lob: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { sessions: rows, pagination: pg } = await listIntakes(lob, {
  ...
  async function handleSearch() {
    ...
    setError(null);
    try {
      setMatches(await searchCrmContacts(lob, crmQuery.trim()));
    } catch (e) {
  ...
  function goToForm(token: string) {
    router.push({ pathname: `${intakeRoute}/[token]` as never, params: { token } });
  }
  ...
  async function startFromContact(c: CrmContactMatch) {
    ...
    setError(null);
    try {
      const session = await createIntake(lob, { crmContactId: c.id, locale });
  ...
  async function createAndStart() {
    ...
    setError(null);
    try {
      const session = await createIntake(lob, { firstName, lastName, email, phone, locale: newLang });
      goToForm(session.token);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Error");
      setBusy(false);
  ...
  function shareUrl(token: string): string {
    ...
    return buildIntakeShareUrl(config, token, locale, origin);
  }
  ...
  async function handleSendLink(s: IntakeSummary) {
    ...
    setError(null);
    try {
      await sendIntakeLink(lob, s.token);
      setSentId(s.id);
  ...
  async function handleReopen(s: IntakeSummary, allow: boolean) {
    ...
    setError(null);
    try {
      const updated = await reopenIntake(lob, s.token, allow);
  ...
  async function handleDelete(s: IntakeSummary) {
    ...
    setError(null);
    try {
      await deleteIntake(lob, s.token);
  ...
  async function handleReset(s: IntakeSummary) {
    ...
    setError(null);
    try {
      const updated = await resetIntakeLink(lob, s.token);
      setSessions((prev) => prev.map((row) => (row.id === s.id ? updated : row)));
                ...
                <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button asChild size="sm" variant="secondary">
                      <Link href={{ pathname: `${intakeRoute}/[token]` as never, params: { token: s.token } }}>
                        <Pencil className="mr-1 h-4 w-4" />
                        {tr(UI.edit, locale)}
                      </Link>
                    </Button>
                    <Button asChild size="sm" variant="outline">
                      <Link href={{ pathname: `${intakeRoute}/[token]/view` as never, params: { token: s.token } }}>
