 ...
 * wants a quote, and the unguessable link plus device binding is what secures the session.
 *
...
export default function IulApplyCta({ label }: { label: string; startHref?: string }) {
  const locale = useLocale().startsWith("es") ? "es" : "en";

  return (
    <Button asChild size="lg" className={CTA_CLS}>
      <a href={`/api/iul-intake/start?locale=${locale}`}>
