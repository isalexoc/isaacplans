...
function displayValue(field: AcaField, raw: unknown, locale: AcaLocale): string {
...
export default function AcaClientView({ token }: { token: string }) {
  ...
  async function handleReopen(allow: boolean) {
    ...
    setReopening(true);
    try {
      const updated = await reopenAcaIntake(session.token, allow);
      setSession((prev) =>
        ...
        <div className="mb-4 flex items-center justify-end gap-3">
          <div className="flex items-center gap-2">
            ...
            <Button asChild size="sm">
              <Link href={{ pathname: "/aca/intake/[token]", params: { token } }}>
