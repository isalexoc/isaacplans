...
import {
  ...
  type FeIntakePagination,
} from "@/lib/fe-intake-api";
import type { FeIntakeSummary, FeIntakeStatus } from "@/lib/fe-intake/types";
...
const STATUS_VARIANT: Record<FeIntakeStatus, "secondary" | "default" | "outline"> = {
  draft: "outline",
  in_progress: "secondary",
  completed: "default",
};

export default function FeIntakeDashboard() {
  const locale = pickLocale(useLocale());
  const router = useRouter();

  const [sessions, setSessions] = useState<FeIntakeSummary[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<FeIntakeStatus | "">("");
  const [page, setPage] = useState(1);
  const [pagination, setPagination] = useState<FeIntakePagination | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { sessions: rows, pagination: pg } = await listFeIntakes({
  ...
  function goToForm(token: string) {
    router.push({ pathname: "/final-expense/intake/[token]", params: { token } });
  }
  ...
  async function startFromContact(c: CrmContactMatch) {
    ...
    setError(null);
    try {
      const session = await createFeIntake({ crmContactId: c.id, locale });
  ...
  async function createAndStart() {
    ...
    setError(null);
    try {
      const session = await createFeIntake({ firstName, lastName, email, phone, locale: newLang });
      goToForm(session.token);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Error");
      setBusy(false);
  ...
  function shareUrl(token: string): string {
    ...
    return buildFeIntakeShareUrl(token, locale, origin);
  }

  async function handleSendLink(s: FeIntakeSummary) {
    setSendingId(s.id);
    setError(null);
    try {
      await sendFeIntakeLink(s.token);
      setSentId(s.id);
  ...
  async function handleReopen(s: FeIntakeSummary, allow: boolean) {
    setReopeningId(s.id);
    setError(null);
    try {
      const updated = await reopenFeIntake(s.token, allow);
  ...
  async function handleDelete(s: FeIntakeSummary) {
    if (!window.confirm(tr(UI.deleteConfirm, locale))) return;
    setDeletingId(s.id);
    setError(null);
    try {
      await deleteFeIntake(s.token);
  ...
  async function handleReset(s: FeIntakeSummary) {
    if (!window.confirm(tr(UI.resetConfirm, locale))) return;
    setResettingId(s.id);
    setError(null);
    try {
      const updated = await resetFeIntakeLink(s.token);
      setSessions((prev) => prev.map((row) => (row.id === s.id ? updated : row)));
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-950">
      <div className="container mx-auto max-w-5xl px-4 py-8">
        ...
        <div className="mt-6 flex flex-wrap items-end gap-3">
          ...
          <select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as FeIntakeStatus | "")}
                ...
                <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button asChild size="sm" variant="secondary">
                      <Link href={{ pathname: "/final-expense/intake/[token]", params: { token: s.token } }}>
                        <Pencil className="mr-1 h-4 w-4" />
                        {tr(UI.edit, locale)}
                      </Link>
                    </Button>
                    <Button asChild size="sm" variant="outline">
                      <Link href={{ pathname: "/final-expense/intake/[token]/view", params: { token: s.token } }}>
