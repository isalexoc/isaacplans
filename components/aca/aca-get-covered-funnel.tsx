...
import { Contact, HeartPulse, Loader2, Phone, Shield } from "lucide-react";
...
import {
  trackAcaGetCoveredAbandon,
  trackAcaGetCoveredFieldCompleted,
  trackAcaGetCoveredFieldStarted,
  trackAcaGetCoveredPhase,
  trackAcaGetCoveredSubmitAttempt,
  trackAcaGetCoveredSubmitSuccess,
  type AcaGetCoveredFieldId,
} from "@/lib/analytics/aca-get-covered-ga";
...
export default function AcaGetCoveredFunnel({
  ...
  const t = useTranslations("acaGetCoveredPage.funnel");
  ...
  const startedFieldsRef = useRef<Set<AcaGetCoveredFieldId>>(new Set());
  const completedFieldsRef = useRef<Set<AcaGetCoveredFieldId>>(new Set());
  ...
  const calendarBookingHref = (() => {
    ...
    return appendAgentCrmBookingPrefill("/aca/calendar", {
  ...
  /** GA4: one event per funnel phase (contact → done). Does not affect Meta Pixel/CAPI. */
  useEffect(() => {
    trackAcaGetCoveredPhase({ phase, locale });
  }, [phase, locale]);
  ...
  useEffect(() => {
    ...
    const trackAbandon = () => {
      ...
      trackAcaGetCoveredAbandon({ locale, time_on_page_seconds: elapsedSeconds });
  ...
  const trackFieldStartedOnce = (fieldId: AcaGetCoveredFieldId) => {
    if (startedFieldsRef.current.has(fieldId)) return;
    startedFieldsRef.current.add(fieldId);
    trackAcaGetCoveredFieldStarted({ field_id: fieldId, locale });
  };

  const trackFieldCompletedOnce = (fieldId: AcaGetCoveredFieldId, isValidValue: boolean) => {
    if (!isValidValue) return;
    if (completedFieldsRef.current.has(fieldId)) return;
    completedFieldsRef.current.add(fieldId);
    trackAcaGetCoveredFieldCompleted({ field_id: fieldId, locale });
  };
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitError(null);
    setFieldErrors({});
    trackAcaGetCoveredSubmitAttempt({ locale });
    ...
    try {
      ...
      trackAcaGetCoveredSubmitSuccess({ locale });
      ...
      if (process.env.NODE_ENV === "development" && capiDispatched !== true) {
        console.warn(
          "[aca/get-covered] Meta CAPI Lead was not dispatched. Set META_CAPI_ACCESS_TOKEN and NEXT_PUBLIC_FACEBOOK_PIXEL_ID; ensure meta.eventId and eventSourceUrl are sent (also sent on duplicate-merge for the aca_get_covered_ads source)."
        );
      }
      ...
      trackLead(
        {
          contentName: "ACA — get covered",
          value: ACA_LEAD_VALUE,
          currency: "USD",
          source: "aca_get_covered_ads",
        },
        eventId
        ...
        <div className="relative hidden overflow-hidden bg-slate-900 lg:sticky lg:block lg:top-0 lg:min-h-[min(100vh,920px)] lg:w-[46%] lg:shrink-0">
          ...
          <div className="absolute inset-x-0 top-[42%] flex -translate-y-1/2 flex-col px-10 xl:px-12">
            <p className="mb-3 inline-flex w-fit items-center gap-2 rounded-full border border-white/25 bg-black/35 px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-white backdrop-blur-md sm:text-xs [text-shadow:0_1px_12px_rgba(0,0,0,0.85)]">
              <HeartPulse className="h-3.5 w-3.5 shrink-0 text-emerald-300" aria-hidden />
              {t("hero.badge")}
            </p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setFirstName(value);
                        trackFieldStartedOnce("first_name");
                        trackFieldCompletedOnce("first_name", value.trim().length > 0);
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const value = e.target.value;
                        setLastName(value);
                        trackFieldStartedOnce("last_name");
                        trackFieldCompletedOnce("last_name", value.trim().length > 0);
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const value = e.target.value;
                        setEmail(value);
                        trackFieldStartedOnce("email");
                        trackFieldCompletedOnce("email", value.trim().length > 0);
                  ...
                  <div className="mt-4 flex w-full flex-col gap-3">
                    <div>
                      <Button
                        ...
                        className="h-16 w-full rounded-xl bg-gradient-to-r from-[hsl(var(--custom))] to-blue-600 text-lg font-semibold text-white shadow-lg"
                      >
                        <Link href={calendarBookingHref as "/aca/calendar"}>
                          {t("done.bookCta")}
                        </Link>
                      </Button>
                    </div>
                    ...
                    >
                      <Link href="/aca/apply">{t("done.applyCta")}</Link>
                    </Button>
                    <Button
                      asChild
                      size="lg"
