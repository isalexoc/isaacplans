/* app/[locale]/final-expense/qualification/page.tsx – server component */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "finalExpenseQualification.meta",
  });
  ...
  const routeKey = "/final-expense/qualification";
...
/* ───────── Page ───────── */
export default async function FinalExpenseQualificationPage() {
  const locale = (await getLocale()) as SupportedLocale;
  const t = await getTranslations({
    locale,
    namespace: "finalExpenseQualification",
  });
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg shadow-lg min-h-[600px] p-8 md:p-12 flex items-center justify-center">
          <div className="w-full max-w-4xl">
            ...
            <div className="mt-8 text-center">
              <p className="text-gray-600 text-lg italic">{t("note")}</p>
            </div>
          </div>
        </div>
      </div>
