...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "finalExpenseGetCoveredPage.metadata",
  });
  ...
  const routeKey = "/final-expense/get-covered";
  ...
  const ogImageUrl = await getEffectiveOgImageUrl("final-expense", "ads", locale);
...
export default async function FinalExpenseGetCoveredAdsPage() {
  const locale = (await getLocale()) as SupportedLocale;
  ...
  const pageLd = getFinalExpenseGetCoveredAdsPageLd(
    locale,
    tMeta("title"),
    tMeta("description")
  );
  const crumbLd = getFinalExpenseGetCoveredAdsBreadcrumbLd(
    locale,
    locale.startsWith("es") ? "Inicio" : "Home",
    tMeta("breadcrumbPage")
  );

  const heroMedia = await getEffectivePageMedia("final-expense", "ads", "hero", locale);
  ...
  return (
    <div className="relative min-h-screen">
      ...
      />
      <FinalExpenseGetCoveredFunnel heroMedia={heroMedia} />
      <script
