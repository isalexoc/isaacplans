/* app/[locale]/contact/calendar/page.tsx – server component */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "contactPage.calendar.meta",
  });
  ...
  const routeKey = "/contact/calendar";
...
export default async function ContactCalendarPage({
  searchParams,
}: PageProps) {
  const locale = (await getLocale()) as SupportedLocale;
  const t = await getTranslations({
    locale,
    namespace: "contactPage.calendar",
  });

  const params = await searchParams;

  /* Booking URL - same as contact page Schedule button (general consultation) */
  const base =
    locale === "es"
      ? "https://link.agent-crm.com/widget/booking/tPPqNMldYmNWIVCyPkHd"
      : "https://link.agent-crm.com/widget/booking/NSjUB0yiEwvZarpmvMcx";
    ...
    <main className="min-h-screen flex flex-col items-center gap-6 p-4">
      <div className="w-full max-w-4xl flex flex-col items-center justify-center">
        <Button asChild variant="secondary">
          <Link href="/contact">{t("backButton")}</Link>
        </Button>
      ...
      <iframe
        src={iframeSrc}
        title="Contact Appointment Calendar"
