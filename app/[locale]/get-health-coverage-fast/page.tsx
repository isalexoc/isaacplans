...
import {
  getHealthCoverageFastAdsBreadcrumbLd,
  getHealthCoverageFastAdsPageLd,
} from "@/lib/seo/jsonld";
import {
  ogLocaleOf,
  localizedSlug,
...
/** Paid-ads landing: same funnel as /get-covered-fast with minimal site header/footer (see middleware + ads-landing). */
export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as SupportedLocale;
  const t = await getTranslations({
    locale,
    namespace: "getHealthCoverageFastAdsPage.metadata",
  });
  ...
  const routeKey = "/get-health-coverage-fast";
...
export default async function GetHealthCoverageFastAdsPage() {
  const locale = (await getLocale()) as SupportedLocale;
  ...
  const pageLd = getHealthCoverageFastAdsPageLd(
    locale,
    tMeta("title"),
    tMeta("description")
  );
  const crumbLd = getHealthCoverageFastAdsBreadcrumbLd(
    locale,
