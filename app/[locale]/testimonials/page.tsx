...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const image = t("image", {
    default: "https://www.isaacplans.com/images/testimonials.png",
  }) as string;
  const alt = t("imageAlt", { default: "Client Testimonials" });

  const routeKey = "/testimonials";
    ...
    <main className="w-full flex flex-col overflow-x-hidden relative">
      ...
      {categories.map((category, index) => {
        const testimonials = Array.from({ length: 12 }, (_, i) => ({
          name: t(`categories.${category.key}.testimonials.${i}.name`),
          text: t(`categories.${category.key}.testimonials.${i}.text`),
        }));
        ...
        return (
          <div key={category.key}>
            <TestimonialSection
              ...
              title={
                <>
                  {t("testimonialSection.titlePrefix")}{" "}
