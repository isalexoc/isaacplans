/* app/[locale]/health-alternative/apply/page.tsx — public Health Coverage Alternative "Apply now" landing (noindex until launched). */
...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("healthAlternativeApply.meta");
  const locale = await getLocale();
  // Admin-overridable social card (lib/page-media); falls back to the message-file image.
  const ogImageUrl = await getEffectiveOgImageUrl("health-alternative", "apply", locale, t("image"));
...
const BENEFIT_ICONS: LucideIcon[] = [Globe, CalendarCheck, Briefcase, Languages];

export default async function HealthAlternativeApplyPage() {
  const t = await getTranslations("healthAlternativeApply");
  const locale = await getLocale();
  // Admin-overridable hero — an image by default, a video once Isaac uploads one.
  const heroMedia = await getEffectivePageMedia("health-alternative", "apply", "hero", locale);
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-950">
      <div className="container mx-auto max-w-4xl px-4 py-10 sm:py-14">
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          <LobApplyCta lob="health-alternative" label={t("cta")} />
