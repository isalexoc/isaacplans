...
/** Paid-ads landing: Health Coverage Alternative lead funnel with minimal site header/footer (middleware + ads-landing). */
export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as SupportedLocale;
  const t = await getTranslations({
    locale,
    namespace: "healthAlternativeGetCoveredPage.metadata",
  });
  ...
  const routeKey = "/health-alternative/get-covered";
...
export default async function HealthAlternativeGetCoveredAdsPage() {
  const locale = (await getLocale()) as SupportedLocale;
  ...
  const pageLd = getHealthAlternativeGetCoveredAdsPageLd(
    locale,
    tMeta("title"),
    tMeta("description")
  );
  const crumbLd = getHealthAlternativeGetCoveredAdsBreadcrumbLd(
    locale,
