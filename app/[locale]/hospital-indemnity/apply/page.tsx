/* app/[locale]/hospital-indemnity/apply/page.tsx — public Hospital Indemnity "Apply now" landing (noindex until launched). */
...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("hospitalIndemnityApply.meta");
  const locale = await getLocale();
  // Admin-overridable social card (lib/page-media); falls back to the message-file image.
  const ogImageUrl = await getEffectiveOgImageUrl("hospital-indemnity", "apply", locale, t("image"));
...
const BENEFIT_ICONS: LucideIcon[] = [Banknote, Wallet, HeartPulse, Languages];

export default async function HospitalIndemnityApplyPage() {
  const t = await getTranslations("hospitalIndemnityApply");
  const locale = await getLocale();
  // Admin-overridable hero — an image by default, a video once Isaac uploads one.
  const heroMedia = await getEffectivePageMedia("hospital-indemnity", "apply", "hero", locale);
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-950">
      <div className="container mx-auto max-w-4xl px-4 py-10 sm:py-14">
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          <LobApplyCta lob="hospital-indemnity" label={t("cta")} />
