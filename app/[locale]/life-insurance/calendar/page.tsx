/* app/[locale]/life-insurance/calendar/page.tsx – server component */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "lifeInsurancePage.calendar.meta",
  });
  ...
  const routeKey = "/life-insurance/calendar";
...
export default async function LifeInsuranceCalendarPage({ searchParams }: PageProps) {
  const locale = (await getLocale()) as SupportedLocale; // "en" | "es"
  const t = await getTranslations({ locale, namespace: "lifeInsurancePage.calendar" });
  ...
  // "Life Insurance" calendar in GHL and hands over the EN/ES widget URLs.
  const base =
    locale === "es"
    ...
    <main className="min-h-screen flex flex-col items-center gap-6 p-4">
      ...
      <div className="w-full max-w-4xl flex flex-col items-center justify-center">
        <Button asChild variant="secondary">
          {/* ✅ locale-aware back link → /en/life-insurance or /es/seguro-de-vida */}
          <Link href="/life-insurance">{t("backButton")}</Link>
        </Button>
      ...
      <iframe
        src={iframeSrc}
        title="Life Insurance Appointment Calendar"
