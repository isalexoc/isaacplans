/* app/[locale]/aca/apply/page.tsx — public ACA "Apply now" landing (noindex until launched). */
...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("acaApply.meta");
  const locale = await getLocale();
  // Admin-overridable social card (lib/page-media); falls back to the message-file image.
  const ogImageUrl = await getEffectiveOgImageUrl("aca", "apply", locale, t("image"));
...
const BENEFIT_ICONS: LucideIcon[] = [ShieldCheck, HeartPulse, Stethoscope, Wallet];

export default async function AcaApplyPage() {
  const t = await getTranslations("acaApply");
  const locale = await getLocale();
  // Admin-overridable hero — an image by default, a video once Isaac uploads one.
  const heroMedia = await getEffectivePageMedia("aca", "apply", "hero", locale);
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-950">
      <div className="container mx-auto max-w-4xl px-4 py-10 sm:py-14">
        ...
        <div className="mt-8 flex flex-col items-center gap-2">
          <LobApplyCta lob="aca" label={t("cta")} />
