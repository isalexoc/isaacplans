...
export async function submitShortTermMedicalForm(
  ...
  try {
    ...
    if (!result.success) {
      const t = await getTranslations({
        locale: isES ? "es" : "en",
        namespace: "guideDetailPage.form",
      });
    ...
    // Use localhost in development so API runs locally and logs are visible; use production URL otherwise
    const baseUrl =
