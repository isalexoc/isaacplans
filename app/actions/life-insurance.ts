...
const LOCALE_LOG = {
  en: "[Life Insurance Lead EN]",
  es: "[Life Insurance Lead ES]",
} as const;

export async function submitLifeInsuranceForm(
  ...
  try {
    ...
    if (!result.success) {
      const t = await getTranslations({
        locale: isES ? "es" : "en",
        namespace: "lifeInsurancePage.leadForm",
      });
    ...
    const response = await fetch(`${baseUrl}/api/create-contact`, {
      ...
      body: JSON.stringify({
        ...
        lifeInsuranceData: {
          language: isES ? "es" : "en",
          source: "life_insurance_page_cta",
