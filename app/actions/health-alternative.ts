...
const LOCALE_LOG = {
  en: "[Health Alternative Lead EN]",
  es: "[Health Alternative Lead ES]",
} as const;

export async function submitHealthAlternativeForm(
  ...
  try {
    ...
    if (!result.success) {
      const t = await getTranslations({
        locale: isES ? "es" : "en",
        namespace: "healthAlternativePage.leadForm",
      });
    ...
    const response = await fetch(`${baseUrl}/api/create-contact`, {
      ...
      body: JSON.stringify({
        ...
        healthAlternativeData: {
          language: isES ? "es" : "en",
          source: "health_alternative_page_cta",
