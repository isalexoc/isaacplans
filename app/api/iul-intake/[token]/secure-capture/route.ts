 ...
 */
function maskedCaptureValues(row: { data: IntakeData | null }): Record<string, string> {
...
export async function POST(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const capture = await createCapture({ sessionId: row.id, ownerUserId: row.ownerUserId });
...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    return NextResponse.json({
      ...
      values: maskedCaptureValues(row),
    });
  } catch (error) {
