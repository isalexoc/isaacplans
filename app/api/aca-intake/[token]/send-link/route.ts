...
// POST /api/aca-intake/[token]/send-link — admin only: refresh the CRM link field + add the
...
export async function POST(_request: NextRequest, context: RouteContext) {
  try {
    ...
    const linkSynced = await syncAcaIntakeLinkToCrm(row);
    if (!linkSynced) {
      return NextResponse.json(
        { success: false, error: "Could not update the link in the CRM. Run pnpm aca:fields and retry." },
        { status: 502 }
      );
    }

    // 2) Remove then re-add the tag so the "tag added" workflow fires on every send.
    await agentCrmRemoveContactTags(row.crmContactId, [ACA_INTAKE_LINK_SENT_TAG], creds.token, "[ACA_INTAKE]");
    const tagged = await agentCrmAddContactTags(row.crmContactId, [ACA_INTAKE_LINK_SENT_TAG], creds.token, "[ACA_INTAKE]");
    if (!tagged) {
    ...
    console.error("[aca-intake/:token/send-link] POST", error);
