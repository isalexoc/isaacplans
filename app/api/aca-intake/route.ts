...
import {
  createAcaIntakeSession,
  listAcaIntakeSessionsForOwner,
  countAcaIntakeSessionsForOwner,
  syncAcaIntakeLinkToCrm,
  toAcaIntakeSummary,
  ACA_INTAKE_PAGE_SIZE,
  ACA_SPANISH_TAG,
  ACA_ENGLISH_TAG,
  type AcaIntakeStatus,
...
const STATUSES: AcaIntakeStatus[] = ["draft", "in_progress", "completed"];

function parseStatus(value: string | null): AcaIntakeStatus | undefined {
  return STATUSES.find((s) => s === value);
}

// GET /api/aca-intake — dashboard list, OR ?contactSearch=… to search the CRM for contacts
export async function GET(request: NextRequest) {
  try {
    ...
    if (contactSearch !== null) {
      ...
      const contacts = await agentCrmSearchContacts(contactSearch, creds.locationId, creds.token, "[ACA_INTAKE]");
    ...
    const limit = Math.max(1, Math.min(parseInt(searchParams.get("limit") ?? String(ACA_INTAKE_PAGE_SIZE), 10) || ACA_INTAKE_PAGE_SIZE, 100));

    const [rows, total] = await Promise.all([
      listAcaIntakeSessionsForOwner(userId, { ...filters, page, limit }),
      countAcaIntakeSessionsForOwner(userId, filters),
    ]);
    const totalPages = Math.max(1, Math.ceil(total / limit));
    return NextResponse.json({
      success: true,
      sessions: rows.map(toAcaIntakeSummary),
      pagination: { page, limit, total, totalPages, hasMore: page < totalPages },
    });
  } catch (error) {
    console.error("[aca-intake] GET", error);
    return NextResponse.json({ success: false, error: "Failed to load" }, { status: 500 });
  }
}

// POST /api/aca-intake — create a session from an existing CRM contact id, or a new contact
export async function POST(request: NextRequest) {
  try {
      ...
      if (creds) {
        ...
        if (crmContactId) {
          await agentCrmAddContactTags(
            crmContactId,
            [locale === "es" ? ACA_SPANISH_TAG : ACA_ENGLISH_TAG],
            creds.token,
            "[ACA_INTAKE]"
          );
        }
      } else {
        console.warn("[aca-intake] Agent CRM credentials missing; creating unlinked session.");
      }
    }
    ...
    await syncAcaIntakeLinkToCrm(row, linkLocaleOverride);

    return NextResponse.json({ success: true, session: toAcaIntakeSummary(row) });
  } catch (error) {
    console.error("[aca-intake] POST", error);
